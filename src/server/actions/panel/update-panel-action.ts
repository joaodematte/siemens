...
export const updatePanelAction = authActionClient.schema(updateSchema).action(async ({ parsedInput, ctx }) => {
  const { id, model, power, manufacturer } = parsedInput;

  const { data: existingPanel } = await ctx.supabase.from('panel').select('*').eq('id', id).single();
