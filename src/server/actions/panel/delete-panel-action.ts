...
export const deletePanelAction = authActionClient.schema(deleteSchema).action(async ({ parsedInput, ctx }) => {
  const { id } = parsedInput;

  const { data: panel } = await ctx.supabase.from('panel').select('*').eq('id', id).single();
