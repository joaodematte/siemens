...
export const createPanelAction = authActionClient.schema(createSchema).action(async ({ parsedInput, ctx }) => {
  const { model, power, manufacturer } = parsedInput;

  const { data: existingPanel } = await ctx.supabase.from('panel').select('*').eq('model', model).throwOnError();
