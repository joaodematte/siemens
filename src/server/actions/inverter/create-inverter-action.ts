...
export const createInverterAction = authActionClient.schema(createSchema).action(async ({ parsedInput, ctx }) => {
  const { model, activePower, manufacturer, inmetroCode } = parsedInput;

  const { data: existingInverter } = await ctx.supabase.from('inverter').select('*').eq('model', model).throwOnError();
