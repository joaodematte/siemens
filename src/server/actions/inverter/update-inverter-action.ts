...
export const updateInverterAction = authActionClient.schema(updateSchema).action(async ({ parsedInput, ctx }) => {
  const { id, model, activePower, manufacturer, inmetroCode } = parsedInput;

  const { data: existingInverter } = await ctx.supabase.from('inverter').select('*').eq('id', id).single();

  if (!existingInverter) {
    throw new Error('Inversor não encontrado.');
  }

  const { data: existingManufacturer } = await ctx.supabase.from('manufacturer').select('*').eq('name', manufacturer);
